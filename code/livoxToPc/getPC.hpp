...
public:
    ...
    //增加点云中的点
    void add(LivoxLidarCartesianHighRawPoint *newPoints, int new_PointsNum)
    {
        //对于每一个点云
        for(int i = 0; i < new_PointsNum; i++)
        {
            this->pc->points.push_back(pcl::PointXYZ(newPoints[i].x / 1000.0f, newPoints[i].y / 1000.0f, newPoints[i].z / 1000.0f));      //添加
    ...
    */
    void updating(LivoxLidarCartesianHighRawPoint *newPoints, int new_PointsNum)
    {
        this->add(newPoints, new_PointsNum);    //增添点云
